...
namespace project
{
    public partial class frmHS_LamBaiThiThu : Form
    {
        ...
        double Diem;
        public frmHS_LamBaiThiThu(string username,string made,string makithi,string tenmh)
        {
            InitializeComponent();
            MaDe = made;
            MaKiThi = makithi;
        ...
        private void frmHS_LamBaiThiThu_Load(object sender, EventArgs e)
        {
        ...
        private void timeThiThu_Tick(object sender, EventArgs e)
        {
            if (SoLuongCauHoi() > 0)
            {
                ThoiGianThi();
        ...
        private void btnDapAn_Click(object sender, EventArgs e)
        {
            if (SoLuongCauHoi() > 0)
            {
                MessageBox.Show(arrDapAnDung[chon].ToString(),"Thông báo",MessageBoxButtons.OK,MessageBoxIcon.Information);
            }
            else
        ...
        public void LuuKetQua()
        {
                ...
                string DiemThi = Diem.ToString();
                string Ngaythi = NgayThi;
                string KiThi = MaKiThi;
                KetQuaThiThu insert = new KetQuaThiThu();
                ...
                db.KetQuaThiThus.InsertOnSubmit(insert);
                try
                {
                    db.SubmitChanges();
                }
